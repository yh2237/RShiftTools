...
public class FfprobeService
{
    ...
    public async Task<MediaInfo?> GetMediaInfoAsync(string filePath)
    {
        ...
        var argList = new List<string> { "-v", "quiet", "-print_format", "json", "-show_format", "-show_streams", filePath };
        using var process = ProcessHelper.StartProcess(_ffprobePath, argList, redirectStdOut: true, redirectStdErr: true);
    ...
    private static MediaInfo? ParseMediaInfo(string filePath, string json)
    {
        ...
        if (format.TryGetProperty("duration", out var dur) && dur.ValueKind == JsonValueKind.String)
            double.TryParse(dur.GetString(), NumberStyles.Float, CultureInfo.InvariantCulture, out durationSeconds);

        var fileSizeBytes = 0L;
        if (format.TryGetProperty("size", out var size) && size.ValueKind == JsonValueKind.String)
            long.TryParse(size.GetString(), NumberStyles.Integer, CultureInfo.InvariantCulture, out fileSizeBytes);
        ...
        foreach (var stream in streams.EnumerateArray())
        {
            ...
            if (codecType == "video" && videoCodec == "")
            {
                ...
                width = stream.TryGetProperty("width", out var w) && w.TryGetInt32(out var wi) ? wi : 0;
                height = stream.TryGetProperty("height", out var h) && h.TryGetInt32(out var hi) ? hi : 0;

                if (stream.TryGetProperty("r_frame_rate", out var fr) && fr.ValueKind == JsonValueKind.String)
                {
            ...
            else if (codecType == "audio" && audioCodec == "")
            {
                ...
                if (stream.TryGetProperty("sample_rate", out var sr) && sr.ValueKind == JsonValueKind.String)
                    int.TryParse(sr.GetString(), NumberStyles.Integer, CultureInfo.InvariantCulture, out audioSampleRate);
            }
        }
